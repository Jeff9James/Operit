...
const UIAutomationSubAgentTools = (function () {
    ...
    const MAIN_SCREEN_LOCK_KEY = '__operit_ui_subagent_main_screen_running';
    ...
    async function run_subagent(params) {
        ...
        if (isMainScreen) {
            try {
                if (globalThis[MAIN_SCREEN_LOCK_KEY]) {
                    return {
                        success: false,
                        message: '主屏模式不支持并发运行 UI 子代理：请等待当前任务结束后再调用。',
                    };
                }
                globalThis[MAIN_SCREEN_LOCK_KEY] = true;
                setCachedAgentId(undefined);
                const result = await Tools.UI.runSubAgent(intent, max_steps, undefined, target_app);
                return {
                    success: true,
                    message: 'UI子代理执行完成',
                    data: result,
                ...
                try {
                    globalThis[MAIN_SCREEN_LOCK_KEY] = false;
                }
                catch (_e) {
                }
            }
